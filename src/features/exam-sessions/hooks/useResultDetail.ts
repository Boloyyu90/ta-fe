 ...
 * Solution: Use getExamSession which returns complete UserExam with all result data
 *
 * Backend returns UserExam with:
...
export function useResultDetail(sessionId: number) {
    return useQuery({
        ...
        /**
         * Use getExamSession instead of non-existent getResultDetail
        ...
        // Optionally add select to extract just the result data
        select: (data) => {
            /**
             * Transform the UserExam response to a result-focused structure
